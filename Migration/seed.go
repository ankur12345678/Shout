...
func SeedDB(DB *gorm.DB) {
	...
	commentRepo := models.InitCommentRepo(DB)
	...
	err = postRepo.Create(&models.Post{PostUUID: "post_1", Title: "Welcome to GOLang", Content: "hi everyone!", UserID: 1, Reputation: 78, Share: 56})
	if err != nil {
		log.Error("err in creating post entry", err)
	}

	err = commentRepo.Create(&models.Comment{CommentUUID: "comment_1", UserID: 1, PostID: 1, Content: "Good tutorial!"})
	if err != nil {
		log.Error("err in creating comment entry", err)
	}
